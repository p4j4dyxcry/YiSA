...
namespace YiSA.Foundation.Operation.Extensions
{
    ...
    public static class OperationExtensions
    {
        ...
        /// </summary>
        public static IOperation SafeExecuteTo(this IOperation @this, IOperator controller)
        {
            if (@this is IMergeableOperation mergeableOperation)
                @this = mergeableOperation.Merge(controller);
            if(@this is IDisposableOperation disposableOperation)
                disposableOperation.BindOperator(controller);
        ...
        public static IOperation SafePushTo(this IOperation @this, IOperator controller)
        {
            if (@this is IMergeableOperation mergeableOperation)
                @this = mergeableOperation.Merge(controller);
            if(@this is IDisposableOperation disposableOperation)
        ...
        {
            var oldValue = InternalPropertyReflection.GetProperty<TProperty>(@this, propertyName);
        ...
        public static IDisposable BindPropertyChangedToOperator<T, TProperty>(this T @this, Expression<Func<T, TProperty>> selector , IOperator @operator,bool autoMergeSpan)
            where T : INotifyPropertyChanged
        {
            return @operator.BindPropertyChanged<TProperty>(@this, selector.GetMemberName(), autoMergeSpan);
        }
        ...
        public static IMergeableOperation GenerateAutoMergeOperation<TProperty,TMergeKey>(this object @this,string propertyName, TProperty newValue ,TProperty oldValue, TMergeKey mergeKey,TimeSpan timeSpan)
        {
            return new MergeableOperation<TProperty>(
                x => { InternalPropertyReflection.SetProperty(@this, propertyName, x); },
                newValue,
        ...
        /// <summary>
        /// メッセージを設定します。
        /// すでにメッセージが指定されている場合上書きします。
        /// </summary>
        /// <param name="operation"></param>
        /// <param name="message"></param>
        /// <typeparam name="T"></typeparam>
        /// <returns></returns>
        public static T SetMessage<T>(this T @operation , string message)
            where T : IOperation
        {
