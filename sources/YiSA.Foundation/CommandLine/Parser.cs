    ...
    internal static class Utility
    {
        public static void Parse(ref object? value, string arg)
        {
            try
            {
                ...
                {
                    int _    => int.Parse(arg),
                    short _  => short.Parse(arg),
                    long _   => long.Parse(arg),
                    uint _   => uint.Parse(arg),
                    ushort _ => ushort.Parse(arg),
                    ulong _  => ulong.Parse(arg),
                    float _  => float.Parse(arg),
                    double _ => double.Parse(arg),
                    decimal _=> decimal.Parse(arg),
                    char _   => char.Parse(arg),
                    string _ => arg,
                    byte _   => byte.Parse(arg),
                    sbyte _  => sbyte.Parse(arg),
                    bool _   => bool.Parse(arg),
                    Enum _   => Enum.Parse(value.GetType(), arg),
                    _ => value
                };
            }
            catch (Exception)
